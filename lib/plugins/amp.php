 ...
 */
function gs_amp_class( $default, $active, $state ) {
	$output = '';
	if( genesis_is_amp() ) {
		$output .= sprintf(
			' [class]="%s"',
 ...
 */
function gs_amp_toggle( $state = '', $disable = array() ) {
	if( ! genesis_is_amp() )
		return;
 ...
 */
function gs_amp_nav_dropdown( $theme_location = false, $depth = 0 ) {
	...
	return gs_amp_toggle( $key ) . gs_amp_class( 'submenu-expand', 'expanded', $key );
}
