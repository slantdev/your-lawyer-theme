...
global $wp_embed;
add_filter( 'gs_the_content', array( $wp_embed, 'run_shortcode' ), 8 );
add_filter( 'gs_the_content', array( $wp_embed, 'autoembed'     ), 8 );
add_filter( 'gs_the_content', 'wptexturize'        );
add_filter( 'gs_the_content', 'convert_chars'      );
add_filter( 'gs_the_content', 'wpautop'            );
add_filter( 'gs_the_content', 'shortcode_unautop'  );
add_filter( 'gs_the_content', 'do_shortcode'       );
 ...
 */
function gs_class( $base_classes, $optional_class, $conditional ) {
 ...
 */
function gs_bg_image_style( $image_id = false, $image_size = 'full' ) {
	if( !empty( $image_id ) )
...
/**
 * display functions for any passed hook
 */
